...
BATCH_SIZE = 30 #Number of frames to hold in memory
...
async def video_stream(websocket: WebSocket):
    ...
    try:
        while 1:
            ...
            # send the image and data to the frontend
        ...
        if len(frame_buffer) > 0:
            print(f"Flushing final {len(frame_buffer)} records to database...")
            db = SessionLocal()
            try:
                db.add_all(frame_buffer)
                db.commit()
            except Exception as e:
                print(f"Database error during final flush: {e}")
            finally:
                db.close()
            frame_buffer.clear()
